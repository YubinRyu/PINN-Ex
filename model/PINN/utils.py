...
def parse_args():
    ...
    parser.add_argument('--dist_url', default='tcp://127.0.0.1:13456', type=str)
...
def train_loss(train_batch, target_batch, device, network, X_lb, X_ub, Y_lb, Y_ub):
    ...
    loss = (loss1 + loss2 + loss3 + loss4 + loss5) * 100 + (loss6 + loss7 / 1000 + loss8)
...
def validation_loss(X_val, Y_val, device, network, X_lb, X_ub, Y_lb, Y_ub):
    ...
    val_loss = (loss1 + loss2 + loss3 + loss4 + loss5) * 100 + (loss6 + loss7 / 1000 + loss8)
...
def plot(train, u_pred, v_pred, w_pred, points_X, points_Y, epoch, dir):
    ...
    plt.savefig(dir + '/result/PINN/adaptive_sampling/contour_plot/{}.png'.format(epoch))
    ...
    PX.to_csv(dir + '/result/PINN/adaptive_sampling/points/X_{}.csv'.format(epoch), index=False, header=False)
    PY.to_csv(dir + '/result/PINN/adaptive_sampling/points/Y_{}.csv'.format(epoch), index=False, header=False)
